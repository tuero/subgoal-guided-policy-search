...
void train_bootstrap(
    ...
    MetricsTracker<ProblemMetrics> metrics_tracker_train(metrics_dir, "train", resume);
    MetricsTracker<ProblemMetrics> metrics_tracker_validate(metrics_dir, "validate", resume);
    ...
    if (resume) {
        ...
        batch_to_skip =
            (metrics_tracker_train.rows.size() - (bootstrap_to_skip * problems_train.size())) / num_problems_per_batch;
    ...
    while (!timer_wall.is_timeout() && bootstrap_iter < max_iterations) {
        ++bootstrap_iter;
