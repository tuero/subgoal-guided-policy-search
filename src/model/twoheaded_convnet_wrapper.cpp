...
TwoHeadedConvNetWrapper::TwoHeadedConvNetWrapper(
    TwoHeadedConvNetConfig config_,
...
auto TwoHeadedConvNetWrapper::SaveCheckpoint(long long int step) -> std::string {
...
auto TwoHeadedConvNetWrapper::SaveCheckpointWithoutOptimizer(long long int step) -> std::string {
    // create directory for model
    std::filesystem::create_directories(path_);
    std::string full_path = absl::StrCat(path_, checkpoint_base_name_, "checkpoint-", step);
    SPDLOG_INFO("Checkpointing model to {:s}.pt", full_path);
...
void TwoHeadedConvNetWrapper::LoadCheckpoint(const std::string &path) {
...
void TwoHeadedConvNetWrapper::LoadCheckpointWithoutOptimizer(const std::string &path) {
    if (!std::filesystem::exists(absl::StrCat(path, ".pt"))) {
...
template <typename InferenceInputT>
    requires IsAny<InferenceInputT, TwoHeadedConvNetWrapper::InferenceInput, TwoHeadedConvNetWrapper::InferenceRefInput>
[[nodiscard]] auto TwoHeadedConvNetWrapper::inference(std::vector<InferenceInputT> &batch)
...
auto TwoHeadedConvNetWrapper::Inference(std::vector<InferenceRefInput> &batch) -> std::vector<InferenceOutput> {
    return inference(batch);
}
